...
def getGradTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15/', model='pathgraphomic_fusion', split='test', use_rnaseq=False, agg_type='mean'):
    ...
    if model == 'path':
        use_patch, roi_dir, use_vgg_features = ('_patch_', 'all_st_patches_512', 0)
    elif (('path' in model) or ('graph' in model)):
        use_patch, roi_dir, use_vgg_features = ('_patch_', 'all_st_patches_512', 1) 
    else:
        use_patch, roi_dir, use_vgg_features = ('_', 'all_st', 0)
    # use_patch, roi_dir, use_vgg_features = ('_patch_', 'all_st_patches_512', 1) if (('pathpath_fusion' in model) or ('pathomic_fusion' in model) or ('graph' in model)) else ('_', 'all_st', 0)
...
def getPredAggGrad_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15/', model='pathgraphomic_fusion', split='test', use_rnaseq=False, 
    ...
    for k in range(1,16):
        ...
        #print(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split))
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))    
        grad_pred = pred[3].T
...
def calcGradMetrics(y_label_all, y_pred_all, avg='micro'):
    ...
    for i in range(15):
        ...
        # print(y_pred)
        # print(avg)
        rocauc_all.append(roc_auc_score(y_label, y_pred, average = avg))
        # rocauc_all.append(roc_auc_score(y_label, y_pred))
...
def makeAUROCPlot(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15/', model_list=['path', 'pathgraphomic_fusion'], split='test', avg='micro', use_zoom=False):
        ...
        for m, model in enumerate(model_list):
            ...
            for k in range(15):
                ...
                if i != avg:
                    ...
                    rocaucs.append(roc_auc_score(y_label, y_pred,average =  avg))
                    tprs.append(interp(mean_fpr, fpr, tpr))
                    tprs[-1][0] = 0.0
    ...
    for i, fig in enumerate(figures):
        ...
        fig.savefig('./Interpretation/TCGA_GBMLGG/Plots/AUC_%s%s.png' % (classes[i], zoom), bbox_inches='tight')
...
def getSurvTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='pathgraphomic_fusion', split='test', use_rnaseq=True, agg_type='Hazard_mean'):
    ...
    if 'agg' in model:
        data_cv_path = './data/TCGA_GBMLGG/splits/gbmlgg15cv_%s_%d_%d_%d%s_agg.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
    else:
        data_cv_path = './data/TCGA_GBMLGG/splits/gbmlgg15cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
    print(data_cv_path)
    ...
    for k in range(1,16):
        ...
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))    
        # print(pred[0:3] + [pred[4]])
        surv_all = pd.DataFrame(np.stack(np.array(pred[0:3] + [pred[4]]))).T
        ...
        assert np.all(np.array(all_dataset_regstrd['Grade']) == pred[4])
        #print('all_dataset',all_dataset.shape)
        #print('all_dataset_regstrd',all_dataset_regstrd.shape)
        all_dataset_regstrd.insert(loc=0, column='Hazard', value = np.array(surv_all['Hazard']))
        #print('all_dataset_regstrd',all_dataset_regstrd['Hazard'])
        hazard_agg = all_dataset_regstrd.groupby('TCGA ID').agg({'Hazard': ['mean', 'median', max]})
        #print('hazard_agg:',hazard_agg)
...
def getPredAggSurv_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='pathgraphomic_fusion', 
    ...
    if 'cox' in model:
        ...
        print(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, 1, use_patch, split))
        for k in range(1,16):
            pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))  
...
def makeHazardSwarmPlot(ckpt_name='./checkpoints/surv_15_rnaseq/', model='path', split='test', zscore=True, agg_type='Hazard_mean'):
    ...
    sns.swarmplot(x = 'Histomolecular subtype', y='Hazard', data=data
, hue='Grade',
    ...
    fig.savefig('./Interpretation/TCGA_GBMLGG/Plots/%s_HSP.png' % (model))
    plt.close()
...
def makeKaplanMeierPlot(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='omic', split='test', zscore=False, agg_type='Hazard_mean', plot_gt=True):
    def hazard2KMCurve(data, subtype, plot_gt=True):
        ...
        def get_name(model):
            mode2name = {'pathgraphomic':'Pathomic F.', 'agg_pathgraph_omic':'Agg Pathomic F.','pathomic':'Pathomic F.', 'graphomic':'Pathomic F.', 'path':'Histology CNN', 'graph':'Histology GCN', 'omic':'Genomic SNN'}
        ...
        plt.legend(fontsize=24, prop=font_manager.FontProperties( style='normal', size=24))
    ...
    fig.savefig('./Interpretation/TCGA_GBMLGG/Plots/%s_KM_%s%s.png' % (model, 'all', '' if plot_gt else '_nogt'), bbox_inches='tight')
    plt.close()
...
def makeKaplanMeierPlot_Baseline(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='Grade'):
    ...
    fig.savefig('./Interpretation/TCGA_GBMLGG/Plots/%s_KM_%s.png' % (model, 'all'), bbox_inches='tight')
...
def getHazardHistogramPlot_KIRC(ckpt_name='./checkpoints/TCGA_KIRC/surv_15/', model='pathgraphomic_fusion', 
    ...
    fig.savefig('./Interpretation/TCGA_KIRC/Plots/%s_HHP.png' % (model))
...
def makeKaplanMeierPlot_KIRC_Binary(ckpt_name='./checkpoints/TCGA_KIRC/surv_15/', model='omic', split='test', zscore=False, agg_type='Hazard_mean', percentile=[50]):
    def hazard2KMCurve(data, percentile):
        ...
        ax.tick_params(axis='both', which='major', labelsize=40)    
        plt.legend(fontsize=32, prop=font_manager.FontProperties(style='normal', size=32))
    ...
    fig.savefig('./Interpretation/TCGA_KIRC/Plots/%s_%s_KM_%s.png' % ('./checkpoints/TCGA_KIRC/surv_15/'.split('/')[2], model, 'all'), bbox_inches='tight')
    plt.close()
...
def makeKaplanMeierPlot_KIRC_Multi(ckpt_name='./checkpoints/TCGA_KIRC/surv_15/', model='omic', split='test', zscore=False, agg_type='Hazard_mean', percentile=[25, 50, 75]):
    def hazard2KMCurve(data, percentile):
        ...
        ax.tick_params(axis='both', which='major', labelsize=40)    
        plt.legend(fontsize=16, prop=font_manager.FontProperties(style='normal', size=16))
    ...
    fig.savefig('./Interpretation/TCGA_KIRC/Plots/%s_%s_KM_%s.png' % ('./checkpoints/TCGA_KIRC/surv_15/'.split('/')[2]+"_Multi", model, 'all'), bbox_inches='tight')
    plt.close()
