...
def CIndex_lifeline(hazards, labels, survtime_all):
    ...
    nan_indices = np.where(np.isnan(hazards))[0]
    
    survtime_all_clean = np.delete(survtime_all, nan_indices, axis=0)
    hazards_clean = np.delete(hazards, nan_indices, axis=0)
    labels_clean = np.delete(labels, nan_indices, axis=0)
    

    return(concordance_index(survtime_all_clean, -hazards_clean, labels_clean))
    '''
