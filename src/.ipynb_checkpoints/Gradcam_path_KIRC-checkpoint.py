...
'''
The code of Gradcam is provided by Chatgpt
'''
...
for k in [1,4,7,10]:
    ...
    train_loader = torch.utils.data.DataLoader(dataset=custom_data_loader, batch_size=20, shuffle=False, collate_fn=mixed_collate)


    checkpoint_path = f'../checkpoints/TCGA_KIRC/surv_15/path/path_{k}.pt'
    ...
    # Load and Plot
    with open(f'../Interpretation/TCGA_KIRC/Gradcam/Gradcam_KIRC_{k}.pkl', 'rb') as file:
    ...
    plt.tight_layout()
    plt.savefig(f'../Interpretation/TCGA_KIRC/Gradcam/Gradcam_KIRC_{k}.png')
