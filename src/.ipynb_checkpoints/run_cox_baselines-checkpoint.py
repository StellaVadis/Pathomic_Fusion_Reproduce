...
def trainCox(dataroot = '../data/TCGA_GBMLGG/', ckpt_name='../checkpoints/surv_15_cox/', model='cox_omic', penalizer=1e-4):
    ...
    model_feats = {'cox_agegender':['Survival months', 'censored','Age at diagnosis','Gender'],
                   'cox_omic':['TCGA ID', 'Histology', 'Grade', 'Molecular subtype', 'Histomolecular subtype'],
    ...
    print(CI_pm(cv_results))
    # print("Average C-Index: %f" % CI_pm(cv_results))

print('0. GENDER AGE. Ignore missing molecular subtypes')
trainCox(model='cox_agegender', penalizer=1e-1)
