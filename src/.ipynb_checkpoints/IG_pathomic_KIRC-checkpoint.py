...
class NewModel(nn.Module):
    ...
    def forward(self, x):
        ...
        _, pred = self.base_model(x_omic=x[:,0:362], x_path = x[:,362:])
...
def main(survive_type):
    ...
    for k in range(1,16,1):
        ...
        checkpoint_path = f'../checkpoints/TCGA_KIRC/surv_15/pathomic_fusion/pathomic_fusion_{k}.pt'
    ...
    sorted_indices = np.argsort(torch.abs((total_attributions).mean(axis = 0)[0:362]).cpu())
    ...
    file_path = f'../Interpretation/TCGA_KIRC/IG/IG_pathomic_{survive_type}.pkl'
    ...
    plt.title(f'IG Contributions for Top Features {survive_type} (Pathomic)')
    plt.grid(True, axis='x', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig(f'../Interpretation/TCGA_KIRC/IG/IG_pathomic_{survive_type}.png')
