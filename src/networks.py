...
def define_net(opt, k):
    ...
    elif opt.mode == "agg_graphomic":
        net = Aggregate_GraphOmicNet(opt=opt, act=act, k=k)
    elif opt.mode == "agg_pathomic":
        net = Aggregate_PathOmicNet(opt=opt, act=act, k=k)
    elif opt.mode == "agg_pathgraphomic":
        net = Aggregate_PathgraphomicNet(opt=opt, act=act, k=k)
...
#############################################################
'''
This is the pre-aggregate Bimodal and Trimodal Fusion.
...
class SelfAttention(torch.nn.Module):
    ...
    def forward(self, x):
        q = self.query(x)  # (B, N, dim)
        k = self.key(x)  # (B, N, dim)
        v = self.value(x)  # (B, N, dim)

        attn = torch.matmul(q, k.transpose(-2, -1)) * self.scale  # (B, N, N)
        attn = torch.nn.functional.softmax(attn, dim=-1)  # (B, N, N)
...
class MultiImageFusion(torch.nn.Module):
    ...
    def forward(self, x):
        # Extract features for each vector
        aggregated_features = self.extract_features(x)
        # Classify
        hazard = self.classifier(aggregated_features)  # (batch_size, 1)
        ...
        return aggregated_features, hazard       
        

    def extract_features(self, x):
        # Extract features for each vector
        batch_size, num_samples, dim = x.shape
        x = x.view(-1, dim)  # (batch_size * num_samples, dim)
        x = self.backbone(x)  # (batch_size * num_samples, 512)

        # Reshape to (batch_size, num_samples, 512)
        x = x.view(batch_size, num_samples, -1)

        # Apply self-attention mechanism
        x = self.attention(x)  # (batch_size, num_samples, 512)

        # Aggregate features (e.g., mean pooling)
        aggregated_features = x.mean(dim=1)  # (batch_size, 512)
        # aggregated_features = x[:,0,:]

        return aggregated_features
    
    
class MultiGraphFusion(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = torch.nn.Sequential(
...
class Aggregate_PathOmicNet(nn.Module):
    def __init__(self,opt,act,k):
        super().__init__()
        self.fusion_model = MultiImageFusion()
        self.omic_net = MaxNet(input_dim=opt.input_size_omic, omic_dim=opt.omic_dim, dropout_rate=opt.dropout_rate, act=act, label_dim=opt.label_dim, init_max=False)
    ...
    def forward(self, **kwargs):
        x = kwargs['x_path']
        path_vec = self.fusion_model.extract_features(x)
        omic_vec, _ = self.omic_net(x_omic=kwargs['x_omic'])
        features = self.fusion(path_vec, omic_vec)
        hazard = self.classifier(features)
        if self.act is not None:
...
class Aggregate_GraphOmicNet(nn.Module):
    def __init__(self,opt,act,k):
        super().__init__()
        self.fusion_model = MultiGraphFusion()
        self.omic_net = MaxNet(input_dim=opt.input_size_omic, omic_dim=opt.omic_dim, dropout_rate=opt.dropout_rate, act=act, label_dim=opt.label_dim, init_max=False)
    ...
    def forward(self, **kwargs):
        x = kwargs['x_grph']
        grph_vec = self.fusion_model.extract_features(x)
        omic_vec, _ = self.omic_net(x_omic=kwargs['x_omic'])
        features = self.fusion(grph_vec, omic_vec)
        hazard = self.classifier(features)
        if self.act is not None:
...
class Aggregate_PathgraphomicNet(nn.Module):
    def __init__(self, opt, act, k):
        super(Aggregate_PathgraphomicNet, self).__init__()
        self.fusion_model1 = MultiImageFusion()  
        self.fusion_model2 = MultiGraphFusion()
        self.omic_net = MaxNet(input_dim=opt.input_size_omic, omic_dim=opt.omic_dim, dropout_rate=opt.dropout_rate, act=act, label_dim=opt.label_dim, init_max=False)
    ...
    def forward(self, **kwargs):
        path_vec = self.fusion_model1.extract_features(kwargs['x_path'])
        grph_vec = self.fusion_model2.extract_features(kwargs['x_grph'])
        omic_vec, _ = self.omic_net(x_omic=kwargs['x_omic'])
        features = self.fusion(path_vec, grph_vec, omic_vec)
        hazard = self.classifier(features)
        if self.act is not None:
