...
def train(opt, data, device, k):
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+opt.niter_decay+1)):
        ...
        for batch_idx, (x_patname, x_path, x_grph, x_omic, censor, survtime, grade) in (enumerate(train_loader)):
...
def test(opt, model, data, split, device):
    ...
    # not compatible with large batch
    if 'agg' in opt.mode:
        test_loader = torch.utils.data.DataLoader(dataset=custom_data_loader, batch_size=1, shuffle=False, collate_fn=mixed_collate)
    else:
    ...
    name_dict = {key: [] for key in data[split]['x_patname']}
    
    for batch_idx, (x_patname, x_path, x_grph, x_omic, censor, survtime, grade) in enumerate(test_loader):

        censor = censor.to(device) if "surv" in opt.task else censor
        grade = grade.to(device) if "grad" in opt.task else grade
        # _, pred = model(x_path=x_path.to(device), x_grph=x_grph.to(device), x_omic=x_omic.to(device))
        
        features, pred = model(x_path=x_path.to(device), x_grph=x_grph.to(device), x_omic=x_omic.to(device))
        

        # if model == 'graph':
        for element in range(0,len(x_patname),1):
            name_dict[x_patname[element]].append(features[element].unsqueeze(0).detach().cpu().numpy())
    ...
    return loss_test, cindex_test, pvalue_test, surv_acc_test, grad_acc_test, pred_test, name_dict
